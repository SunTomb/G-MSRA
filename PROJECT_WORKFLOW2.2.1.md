...
PYTHONPATH=/NAS/yesh/G-MSRA CUDA_VISIBLE_DEVICES=1 python scripts/eval_locomo.py \
    --checkpoint outputs_v1/phase3_v11/checkpoint_500 \
    --lora_checkpoint outputs_v1/phase1/best \
    --no_qlora --benchmark locomo \
    --output_dir results_v1/diag_ckpt500_phase1lora \
    2>&1 | tee logs_v1/diag_ckpt500_phase1lora.log
```
...
**下载**：`results_v1/diag_ckpt500_phase1lora/locomo_results.json`
...
model = PeftModel.from_pretrained(base_model, "outputs_v1/phase1/best")
phase1_state = {k: v.clone() for k, v in model.state_dict().items() if "lora" in k}

model = PeftModel.from_pretrained(base_model, "outputs_v1/phase3_v11/best/lora")
...
| 路径 | 说明 |
|------|------|
| `results_v1/gmsra_v11/` | A. G-MSRA v11 完整系统 |
| `results_v1/gmsra_v11_ckpt_only/` | B. checkpoint-only |
| `results_v1/events_only_v11/` | C. Events Only baseline |
| `results_v1/no_memory_v11/` | D. No Memory |
| `results_v1/phase2_v7/` | E. Phase 2 v7 |
| `results_v1/diag_ckpt500/` | 诊断：ckpt500 + v11 LoRA |
| `results_v1/diag_ckpt2000/` | 诊断：ckpt2000 + v11 LoRA |
| `results_v1/diag_lora_only/` | 诊断：空 mem + v11 LoRA |
| `results_v1/diag_ckpt500_only/` | 诊断：ckpt500 memory only |
| `results_v1/ablation_ckpt_500/` | Ablation：ckpt500 |
| `results_v1/ablation_ckpt_1000/` | Ablation：ckpt1000 |
| `results_v1/ablation_ckpt_1500/` | Ablation：ckpt1500 |
| `results_v1/ablation_ckpt_2000/` | Ablation：ckpt2000 |
| `results_v1/diag_ckpt500_phase1lora/` | **方案 B 验证**（待完成） |
